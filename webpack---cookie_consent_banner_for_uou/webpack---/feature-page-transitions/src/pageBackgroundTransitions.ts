	...
	(
		...
		const unsubscribeFromPropChanges = propsStore.subscribeToChanges((changes) => {
			if (pageBackgroundCompId in changes) {
				...
				const shouldFixPageBackgroundLayoutInNavigation =
					experiments['specs.thunderbolt.compCssMappers_catharsis']
				if (!transitionEnabled && pageConfig.isEditor && shouldFixPageBackgroundLayoutInNavigation) {
					const htmlElement = document.querySelector('[id^="pageBackground_"]')
					if (htmlElement) {
						htmlElement.id = pageBackgroundCompId
					}
				}
				propsStore.update({
					BACKGROUND_GROUP: {
